...
class BaseApi:
    ...
    def _patch(self, url, payloads, headers=None):
        if not headers:
            headers = self.__headers
        response = self.__requests.patch(f'{self.__base_url}{url}', payloads, headers=headers)
        return response

    def _put(self, url, payloads, headers=None):
        if not headers:
            headers = self.__headers
        response = self.__requests.put(f'{self.__base_url}{url}', payloads, headers=headers)
        return response
